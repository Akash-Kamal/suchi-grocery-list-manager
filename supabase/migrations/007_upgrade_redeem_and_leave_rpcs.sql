-- ============================================================
-- Suchi Grocery App — Atomic Leave & Join RPC Upgrade
-- Migration 007: Atomic Household Switching & Leave Current RPC
-- ============================================================

-- ─── 1. RPC: leave_current_household ──────────────────────────
...
begin
  ...
  if not found then
    return json_build_object('success', true, 'message', 'Not in any household.');
  end if;

  return public.leave_household(v_household_id);
end;
$$;

grant execute on function public.leave_current_household() to authenticated;

-- ─── 2. Upgrade redeem_household_invite with p_leave_current ──
...
begin
  ...
  if found then
    if p_leave_current = true then
      -- Atomically leave existing household before joining the new one
      perform public.leave_household(v_other_membership.household_id);
    else
